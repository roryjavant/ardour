...
RegionFactory::create (boost::shared_ptr<const Region> region, bool announce)
{
	...
	if ((ar = boost::dynamic_pointer_cast<const AudioRegion>(region)) != 0) {
		...
		ret = boost::shared_ptr<Region> (new AudioRegion (ar, AudioMusic (0, 0.0)));
		...
		if (mr->session().config.get_midi_copy_is_fork()) {
			...
			ret = boost::shared_ptr<Region> (new MidiRegion (mr, AudioMusic (0, 0.0)));
		}
...
boost::shared_ptr<Region>
RegionFactory::create (boost::shared_ptr<Region> region, AudioMusic offset, const PropertyList& plist, bool announce)
{
	boost::shared_ptr<Region> ret;
	boost::shared_ptr<const AudioRegion> other_a;
	boost::shared_ptr<const MidiRegion> other_m;
